...
export function createUpdateSplitRoute() {
	return new Hono().patch('/:id', validate('json', createSplitSchema), requirePermission('splits:write'), uuidParam('id'), async c => {
		...
		// Use createOrUpdate with the split ID to update including items
